...
- **Database** — Managed PostgreSQL: N/A (we use SQLite + Telegram). PITR: Equivalent (Telegram mirror is append-only, replayable). Backups: Implemented (every record mirrored; manifest pinned). Triggers: Equivalent (event system fires \`record:changed\` via WebSocket + Telegram mirror). SQL editor / branching / functions / views / materialized views: Roadmap. Read replicas / connection pooling / FDW: N/A (embedded SQLite).
