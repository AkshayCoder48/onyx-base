...
| POST | /api/v1/graphql | Bearer | GraphQL query (records, collections, apiKeys, logs, me) — returns \`{ data, errors }\` |
| POST | /api/v1/rpc/:name | Bearer | Built-in RPC: \`count_records\`, \`sum\`, \`aggregate\`, \`search\`, \`touch\` |
| GET · POST · DELETE | /api/v1/views · /api/v1/views/:name | Bearer | Named projections over a collection |
| GET · POST · DELETE | /api/v1/functions · /api/v1/functions/:name | Bearer | Server-side JS handlers (sandboxed \`new Function(ctx, code)\`) |
| GET · POST · DELETE | /api/v1/matviews · /api/v1/matviews/:name | Bearer | Materialised views (cached SELECT results; refresh on demand) |
| GET  | /api/openapi.json | none | OpenAPI 3.0 spec for all /v1/* + /api/auth/* + /api/v1/* routes |
| GET  | /api/docs | none | Swagger UI (loads /api/openapi.json from the official CDN) |
| GET · POST | /api/admin/network | Bearer \`onyxbase_*\` | Get / mutate the runtime IP allowlist |
| GET · POST · DELETE | /api/admin/branches · /api/admin/branches/:name | Bearer \`onyxbase_*\` | DB branching — snapshot & restore SQLite + JSON cache |
...
- **Security** — RLS: per-userId data isolation on every query (Equivalent). Policies: API-key scoping + share-token scope field (Equivalent). JWT verify: signed download tokens, HMAC-SHA256 constant-time (Equivalent). SSL: HTTPS at Caddy gateway + HSTS (Equivalent). Vault: all secrets in .env, gitignored (Equivalent). Audit logs: every write/login/admin action in \`logs\` table + Telegram mirror (Implemented). Network restrictions / IP allow-lists: Implemented (\`IP_ALLOWLIST\` env + runtime allowlist via POST /api/admin/network; IPv4 CIDR matching; enforced in src/proxy.ts on every /v1/* and /api/* request; empty = open).
- **Database** — Managed PostgreSQL: N/A (we use SQLite + Telegram). PITR: Equivalent (Telegram mirror is append-only, replayable). Backups: Implemented (every record mirrored; manifest pinned). SQL editor: Implemented (read+write SELECT/INSERT/UPDATE/DELETE/CREATE/DROP/ALTER against user-scoped virtual tables — records, collections, api_keys, logs, users — 1000-row cap, API keys masked, custom usr_* tables allowed). Triggers: Equivalent (event system fires \`record:changed\` via WebSocket + Telegram mirror). Branching: Implemented (admin POST /api/admin/branches snapshots SQLite + JSON cache; restore + delete supported). Functions (PL/pgSQL equivalent): Implemented (POST /api/v1/functions stores JS code; POST /api/v1/functions/:name test-invokes in a \`new Function(ctx, code)\` sandbox with { record, db, user }; 5s timeout; db is read-only + user-scoped). Views: Implemented (POST /api/v1/views { name, collection, projection, filter? }; GET /api/v1/views/:name executes the projection). Materialised views: Implemented (POST /api/v1/matviews { name, query } runs + caches the result; GET reads O(1); POST refreshes; refresh-all via POST /api/v1/matviews { action: "refresh_all" }). Read replicas / connection pooling / FDW: N/A (embedded SQLite).
- **Data API** — REST API: Implemented (\`/v1/*\`). Auto-generated RESTful API: every collection auto-exposes \`/v1/set\` \`/v1/get\` \`/v1/delete\` \`/v1/list\` (Implemented). Realtime API: WebSocket on :3003 pushes \`record:changed\` (Implemented). API keys: \`kv_live_*\`, per-user, revocable, named (Implemented). JWT auth: signed download tokens (Equivalent). Automatic RLS-by-default: every new collection is immediately accessible via \`/v1/*\` with per-userId isolation (Equivalent). GraphQL: Implemented (POST /api/v1/graphql — minimal subset: records, collections, apiKeys, logs, me; user-scoped; \`{ data, errors }\` response). OpenAPI docs: Implemented (GET /api/openapi.json + GET /api/docs Swagger UI). RPC: Implemented (POST /api/v1/rpc/:name — \`count_records\`, \`sum\`, \`aggregate\`, \`search\`, \`touch\`).
